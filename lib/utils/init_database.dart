    ...
    for (final item in yamlList) {
      if (!shareBox.values.any((s) => s.uploaderUrl == item["RequestURL"])) {
        shareBox.add(Share(
          ...
          uploadHeaders: Map<String, String>.from(yamlMapToMap(item["Headers"] ?? {})),
          uploadParameters: Map<String, String>.from(yamlMapToMap(item["Parameters"] ?? {})),
          uploadArguments: Map<String, String>.from(yamlMapToMap(item["Arguments"] ?? {})),
